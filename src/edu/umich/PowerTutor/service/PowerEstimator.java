...
import au.csiro.appmonitor.AppMonitor;
...
public class PowerEstimator implements Runnable {
  ...
  private static final String[] app2montior =AppMonitor.PROCESSNAMES;
        ...
        for(int i = 0; i < components; i++) {
          ...
          for(int j = 0; j < uidPower.size(); j++) {
            ...
            if(uid < SystemInfo.AID_APP) {
              ...
              for (int n=0; n<app2montior.length;n++){
            	  if (newAppId.contains(app2montior[n]))
...
//                                   avgPower);
      ...
      /* Update the widget. 
       * 
       * Commented by prem as we do not need widgets
       * */
//      if(iter % 60 == 0) {
...
//      }
     ...
     * 
     * Commented by prem as we do not need a widget button
     * */
